...
bool SceneParser::parse(std::string filepath, RenderData &renderData) {
    ...
    renderData.shapes.clear(); // datatype of renderData.shapes is
                               // std::vector<RenderShapeData>
...
void SceneParser::parseHelper(SceneNode *currentNode, RenderData &renderData,
                              glm::mat4 ctm) {
    ...
    if (currentNode->children.size() == 0) {
        for (int i = 0; i < currentNode->primitives.size(); i++) {
            ...
            renderShapeData = RenderShapeData{
                primitive,
                ctm}; // ctm is the same for every primitive in this node
    ...
    */
    ...
    // this is non-leaf nodes, but if there're primitives, add to renderData.
    // cmt is already updated
    for (auto i = 0; i < currentNode->primitives.size(); i++) {
        ScenePrimitive primitive = *(currentNode->primitives[i]);
        renderShapeData = RenderShapeData{
            primitive,
            ctm}; // these primitives are in the same node so have the same ctm
        renderData.shapes.push_back(renderShapeData);
    }
...
glm::mat4
SceneParser::getCTM(std::vector<SceneTransformation *> &transformations) {
    glm::mat4 matrix(1.f);
    glm::mat4 cmt(1.f);
    ...
    for (int i = 0; i < transformations.size(); i++) {
        ...
        case TransformationType::TRANSFORMATION_ROTATE:
            matrix = glm::rotate(currentTransformation->angle,
